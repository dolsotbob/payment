...
// ❗️ 가스리스 버튼 생겼지만, 이 코드는 백업/비상용 버튼으로 남겨두기 
...
const ApproveAndPay: React.FC<ApproveAndPayProps> = ({ account, amount }) => {
    const handleApproveAndPay = async () => {
        try {
            ...
            // 6. 현재 allowance 확인 후 approve 필요 시 처리 
            ...
            try {
                const cashbackRate = await paymentContract.cashbackRate(); // uint256 반환됨
                cashbackAmount = ethers.formatUnits(
                    (weiAmount * cashbackRate) / 100n,
                    18
                );
            } catch (error) {
                console.warn('⚠️ 캐시백 비율 조회 실패, 기본값 0 사용', error);
            }

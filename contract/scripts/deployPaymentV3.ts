...
async function main() {
    console.log(`\n=== Deploy PaymentV3 (implementation) on ${network.name} ===`);

    // 1) PaymentV3 팩토리 취득
    const PaymentV3 = await ethers.getContractFactory("PaymentV3");

    // 2) 로직 컨트랙트 배포 (UUPS 구현 컨트랙트 — 프록시 아님)
    const impl = await PaymentV3.deploy();
    await impl.waitForDeployment();

    const implAddress = await impl.getAddress();
    console.log(`PaymentV3 implementation deployed at: ${implAddress}`);
